...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) {
      throw new AppError('Could not find this costumer');
    }
    ...
    const unavailableQuantity = products.filter(
      product =>
        existentProducts.filter(p => p.id === product.id)[0].quantity <
        product.quantity,
    );

    if (unavailableQuantity.length) {
      throw new AppError(
        `The quantity ${unavailableQuantity[0].quantity} is not available for ${unavailableQuantity[0].id}`,
      );
    }
